...
vector<double> NeuralNetwork::predict(DataInstance instance) {
    ...
    while(!q.empty()){
        ...
        for (int i = 0; i < levelSize; i++) {
            ...
            for (auto& neighbor : adjacencyList.at(nodeId)) {
                Connection c = neighbor.second;
                visitPredictNeighbor(c);
                nextLayer.insert(c.dest);
            }
        }
...
bool NeuralNetwork::update() {
    double denominator = batchSize == 0 ? 1 : batchSize;
    for (int i = 0; i < nodes.size(); i++) {
        if (nodes[i]!= nullptr) {
            nodes[i]->bias -= learningRate * (nodes[i]->delta / denominator);
            nodes[i]->delta = 0;
        }
    }
    ...
    for (int i = 0; i < adjacencyList.size(); i++) {
        for (auto& neighbor : adjacencyList[i]) {
            Connection& c = neighbor.second;
            c.weight -= learningRate * (c.delta / denominator);
            c.delta = 0;
        }
    }
